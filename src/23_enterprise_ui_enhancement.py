...
APP_ENVIRONMENT = os.getenv(
    "HEVEMIND_ENVIRONMENT",
    "Validated Research Environment",
).strip()

APP_SITE = os.getenv(
    "HEVEMIND_SITE",
    "Malaysia Engineering Demonstration",
).strip()

APP_VERSION = os.getenv(
    "HEVEMIND_APP_VERSION",
    "1.0.0",
).strip()
...
<style>
    ...
    .enterprise-header {
        ...
        background:
            linear-gradient(
                110deg,
                rgba(7, 20, 38, 0.98) 0%,
                rgba(18, 50, 82, 0.98) 70%,
                rgba(31, 106, 165, 0.98) 100%
            );
        border: 1px solid rgba(255, 255, 255, 0.10);
        border-radius: 18px;
        padding: 1.1rem 1.3rem;
        margin-bottom: 1rem;
        box-shadow: 0 14px 34px rgba(7, 20, 38, 0.20);
    ...
    .sidebar-user-card {
        ...
        background:
            linear-gradient(
                145deg,
                rgba(23, 69, 111, 0.96) 0%,
                rgba(18, 50, 82, 0.96) 100%
            );
    ...
    .workflow-stage {
        ...
        background: #ffffff;
        border: 1px solid var(--slate-200);
        box-shadow: 0 5px 16px rgba(21, 48, 75, 0.05);
    }
...
authenticated_name = display_value(
...
authenticated_role = display_value(
    str(
        authenticated_user.get(
            "role",
            "viewer",
...
st.sidebar.markdown(
    ...
    <div class="sidebar-user-card">
        <div class="sidebar-user-label">Signed in user</div>
        <div class="sidebar-user-name">{authenticated_name}</div>
        <div class="sidebar-user-role">Access role: {authenticated_role}</div>
    </div>

    <div class="sidebar-system-grid">
        <div class="sidebar-system-card">
            <div class="sidebar-system-label">API</div>
            <div class="sidebar-system-value">
                {"Healthy" if api_healthy else "Degraded"}
            </div>
        </div>
        <div class="sidebar-system-card">
            <div class="sidebar-system-label">Reports</div>
            <div class="sidebar-system-value">{available_reports}</div>
        </div>
        <div class="sidebar-system-card">
            <div class="sidebar-system-label">Environment</div>
            <div class="sidebar-system-value">{APP_ENVIRONMENT}</div>
        </div>
        <div class="sidebar-system-card">
            <div class="sidebar-system-label">Version</div>
            <div class="sidebar-system-value">{APP_VERSION}</div>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
# GLOBAL SEARCH
# ============================================================
global_search_columns = st.columns(
    [
        1.45,
        0.55,
    ]
...
with global_search_columns[0]:
    global_search_text = st.text_input(
        "Global wafer search",
        placeholder=(
            "Search by complete or partial wafer ID"
        ),
        key="global_wafer_search",
    )

with global_search_columns[1]:
    st.markdown(
        """
        <div class="small-muted" style="padding-top:1.85rem;">
            Search is limited to validated wafer identifiers currently
            available through the deployment API.
        </div>
        """,
        unsafe_allow_html=True,
    )

if global_search_text.strip():
    matching_global_records = all_wafers_df.loc[
        all_wafers_df[
            "wafer_id"
        ]
        .astype(str)
        .str.contains(
            global_search_text.strip(),
            case=False,
            regex=False,
        )
    ].copy()

    if matching_global_records.empty:
        st.info(
            "No validated wafer identifier matches the current search."
        )

    else:
        selected_global_wafer = st.selectbox(
            "Matching wafer",
            options=matching_global_records[
                "wafer_id"
            ].astype(str).tolist(),
            key="global_selected_wafer",
        )

        global_result = matching_global_records.loc[
            matching_global_records[
                "wafer_id"
            ].astype(str)
            == selected_global_wafer
        ].iloc[
            0
        ]

        decision_value = display_value(
            global_result.get(
                "uncertainty_adjusted_decision"
            )
        )
        ...
        st.markdown(
            ...
            <div class="evidence-note">
                <strong>{selected_global_wafer}</strong>
                &nbsp;
                <span class="decision-badge {badge_class}">
                    {decision_value}
                </span>
                <br>
                Failure probability:
    ...
    st.markdown(
        ...
        <div class="workflow-strip">
            <div class="workflow-stage">
                <div class="workflow-stage-index">Stage 01</div>
                <div class="workflow-stage-name">Primary prediction</div>
            </div>
            <div class="workflow-stage">
                <div class="workflow-stage-index">Stage 02</div>
                <div class="workflow-stage-name">Probability calibration</div>
            </div>
            <div class="workflow-stage">
                <div class="workflow-stage-index">Stage 03</div>
                <div class="workflow-stage-name">Uncertainty assessment</div>
            </div>
            <div class="workflow-stage">
                <div class="workflow-stage-index">Stage 04</div>
                <div class="workflow-stage-name">Historical evidence</div>
            </div>
            <div class="workflow-stage">
                <div class="workflow-stage-index">Stage 05</div>
                <div class="workflow-stage-name">Sensor prioritisation</div>
            </div>
            <div class="workflow-stage">
                <div class="workflow-stage-index">Stage 06</div>
                <div class="workflow-stage-name">Engineering recommendation</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
